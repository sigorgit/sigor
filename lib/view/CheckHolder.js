...
class CheckHolder {
    ...
    async checkDiscordLogin() {
        ...
        if (code !== null) {
            try {
                await superagent_1.default.get("https://api.sigor.com/discord/token").query({
                    code,
        ...
        if (code !== undefined) {
            try {
                const result = await superagent_1.default.get("https://api.sigor.com/discord/me").query({ code });
